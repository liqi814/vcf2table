...
# drop columns without sentinel snp
columns2drop = phased_vcf.columns[(phased_vcf[phased_vcf['ID'] == "3-44902386-T-A"] == '0|0').iloc[0]]
phased_vcf.drop(columns=columns2drop, inplace = True)

for sample_name in phased_vcf.columns[5:]:
    if phased_vcf.loc[50, sample_name] == '0|1':
...
tmp_phased_vcf = phased_vcf[phased_vcf.columns[~phased_vcf.columns.isin(['CHROM', 'POS', 'REF'])]]
tmp_phased_vcf.sort_values(by = ['3-44756245-T-C', '3-44743577-T-C', '3-44811166-G-GT', '3-44775257-T-G'], axis = 1, ascending = False, inplace=True)
...
QV_phased_vcf = QV_phased_vcf[QV_phased_vcf.columns[~QV_phased_vcf.columns.isin(['CHROM', 'ID', 'REF'])]]
...
topSNPs_phased_vcf = topSNPs_phased_vcf[topSNPs_phased_vcf.columns[~topSNPs_phased_vcf.columns.isin(['CHROM', 'ID', 'REF'])]]
...
positions = [44720000, 44740000, 44760000, 44780000, 44800000, 44820000, 44840000, 44860000, 44880000, 44900000]
labels = ['44,720,000', '44,740,000', '44,760,000', '44,780,000', '44,800,000', '44,820,000', '44,840,000', '44,860,000', '44,880,000', '44,900,000']
...
fig.savefig('test_QV_longIPFlist.pdf', dpi=200)
