...
for sample_name in phased_vcf.columns[5:]:
    if phased_vcf[sample_name].value_counts()['0|0'] > 49:
...
QV_phased_vcf = QV_phased_vcf.iloc[:, [1] + list(range(5,203))]
...
for QV_sample in QV_phased_vcf["Sample Name"]:
    topSNPs_phased_vcf.insert(len(topSNPs_phased_vcf.columns)-1, QV_sample, topSNPs_phased_vcf.pop(QV_sample))

topSNPs_phased_vcf = topSNPs_phased_vcf.iloc[:, [1] + list(range(5,203))]
